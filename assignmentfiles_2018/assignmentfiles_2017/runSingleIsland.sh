...
run="javac -cp contest.jar Group35.java Individual.java Population.java Island.java Archipelago.java Printing.java EvaluationCounter.java && jar cmf MainClass.txt submission.jar Group35.class Individual.class EvaluationCounter.class Population.class Island.class Archipelago.class Printing.class &&
     java -DnIslands=1 -DpopulationSize=$3 -DoffspringSize=$4 -DreprodStrat=$5 -DparentSelectStrat=$6 -DrecombStrat=$7 -DmutateStrat=$8 -DsurvivorSelectionStrat=$9 -Dalfa=${10} -Dk=${11} -DmutationRate=${12} -Dmean=${13} -DstdDeviation=${14} -Ds=${15} -Devaluation=$2 -jar testrun.jar -submission=Group35 -evaluation=$2 -seed=1"
...
for i in $(seq 1 $END);
    do mkdir -p "data/$2/single_island/$TIMEDATE" && echo "Run $i..." && eval $run > "data/$2/single_island/$TIMEDATE/run$i.txt";
done
