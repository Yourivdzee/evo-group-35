...
df = pd.read_table(dir_loc + file_name, skipfooter=4, sep = " ", index_col=0, error_bad_lines = False, header=6)
df = df[:-2]
...
print(dir_loc)
save_dir = file_name.split("/ru")[0] 
print("save dir  " + save_dir + "/" + "{}.png".format(save_dir).replace("/",""))
plt.savefig(save_dir + "/" + "{}.png".format(save_dir).replace("/",""), dpi = 300)
